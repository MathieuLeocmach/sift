...
\begin{figure}
	...
	\begin{axis}[%
		...
		\addplot table [x expr ={2*\thisrow{r}/1.23}, y index=1] {all_ico_icongb_mrco_X.rdist};
...
\begin{figure*}
	...
	\end{axis}
	\node[lab] at (errorsize.outer south west) {a};
	...
	\end{axis}
	\node[lab] at (dist_er.outer south west) {b};
	...
	\end{axis}
	\node[lab] at (near_R.outer south west) {c};
	...
	\end{axis}
	\node[lab] at (finite.outer south west) {d};
	\end{tikzpicture}
...
\begin{figure}
	...
	\begin{tikzpicture}[
		...
		\nextgroupplot[xmin=0,xmax=511,ymin=0,ymax=511,%
			...
			\draw[ultra thick, white] (axis cs:25,490) -- (axis cs:125,490)  node[midway, below]{200\micro\metre};
		...
		\end{groupplot}
	\end{tikzpicture}
	\caption{\textbf{Other application.} \textbf{a} Fluorescent droplets in a microfluidic device. \textbf{b} Crystal nucleation under phase contrast microscope. The circles are the result of our algorithm.}
...
\begin{figure}
	...
	\begin{tikzpicture}[
		...
		\matrix[matrix of nodes, inner sep=1pt]%
		{%
			\includegraphics[width=0.3\columnwidth]{anim_small_boo000.png} & %
			\includegraphics[width=0.3\columnwidth]{anim_small_boo060.png} & %
			\includegraphics[width=0.3\columnwidth]{anim_small_boo120.png}\\
			\includegraphics[width=0.3\columnwidth]{anim_small_boo180.png} & %
			\includegraphics[width=0.3\columnwidth]{anim_small_boo240.png} & %
			\includegraphics[width=0.3\columnwidth]{anim_small_boo299.png}\\%
		};
	\end{tikzpicture}
	\caption{\textbf{Heterogeneous nucleation.} Successive computer reconstruction of the first three layers from the wall, every \unit{12}{\hour}, seen from the fluid side. Particles sizes are according to our method. Small ($R<\unit{1.5}{\micro\metre}$) particles are shown in green. Other particles are coloured according to their crystal-like order, from blue ($Q_6=0$) to red ($Q_6>0.55$). Note how the small particles are expelled from the forming crystallites to concentrate in the grain boundaries.}
