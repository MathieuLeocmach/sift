...
In dense phases, the neighbouring particles influence the scale dependence of DoG response. This effectively shifts the minima of the DoG towards smaller scales, leading to smaller radii if one uses Eq.~(\ref{eq:scale_dil}) alone. Furthermore, the particles in an experimental image are not uniformly bright due to synthesis imperfection (quantity of dye fixed by each particle) and photo bleaching. If one does not take into account the relative brightness of the particles the less bright will appear even smaller. In the following, we detail our method to take into account the influence of neighbouring particles.

Assuming once again binary ball objects of radii $\lbrace R_i\rbrace$, intensities $\lbrace A_i\rbrace$ and separated by distances $\lbrace r_{ij}\rbrace$, one can superpose the responses at each particle center, using the function $DoG(r,R,\sigma)$ derived in Appendix~\ref{sec:gaussian_vs_ball}:
\begin{equation}
\forall i,\quad DoG_i(\lbrace A_j\rbrace, \lbrace r_{ij}\rbrace, \lbrace R_j\rbrace, \sigma) = \sum_j A_j DoG(r=r_{ij}, R=R_j, \sigma)
\label{eq:superpos}
\end{equation}
This system is linear in $\lbrace A_i\rbrace$, and thus one can easily obtain $\lbrace A_i\rbrace$ when the $\lbrace R_i\rbrace$ are given ($\lbrace r_{ij}\rbrace$, $\lbrace \sigma_i \rbrace$ and $\lbrace DoG_i(\sigma_i)\rbrace$ are known from the \textsc{sift}).

We also know that for all $i$, $DoG_i$ is minimum with respect to $\sigma$ at $\sigma_i$ by construction, thus 
\begin{equation}
\forall i,\quad \frac{\partial DoG_i}{\partial\sigma}(\lbrace A_j\rbrace, \lbrace r_{ij}\rbrace, \lbrace R_j\rbrace, \sigma_i) = 0
\label{eq:DoG_min}
\end{equation}

The system defined by Eq.~(\ref{eq:DoG_min}) is non linear but can be solved iteratively by Newton's method:
\begin{equation}
...
\label{eq:Newton}
\end{equation}
where the matrice and the right hand side are computed for $(\lbrace A_j\rbrace^{(k)}, \lbrace r_{ij}\rbrace, \lbrace R_j\rbrace^{(k)}, \sigma_i)$, with the parenthesised indices indicating the iteration rank.

Given Eq.~(\ref{eq:superpos}), the elements of the matrix simplify to
\begin{equation}
\frac{\partial^2 DoG_i}{\partial R_j\partial\sigma} =  A_j^{(k)} \frac{\partial^2 DoG}{\partial R\partial\sigma}(r=r_{ij}, R=R_j^{(k)}, \sigma=\sigma_i)
\end{equation}

We propose the following algorithm to determine $\lbrace A_i\rbrace$ and $\lbrace R_i\rbrace$:
\begin{enumerate}
...
In principle both Eq.~(\ref{eq:superpos}) and Eq.~(\ref{eq:Newton}) are very large systems of equations ($N\times N$ system, with $N$ the number of particles). However the DoG functions and its derivatives are rapidly decaying functions, thus the matrices are actually very sparse (about as many non-zero coefficients as particles in the first coordination shell), alleviating dramatically the computational burden when using sparse system solvers. Furthermore, the results converge very quickly (about two iterations in our case).

In synthetic images we measured the error in the resulting radii to be around $1\%$. 
