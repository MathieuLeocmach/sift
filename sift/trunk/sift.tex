...
We used the same construction algorithm as \cite{Lowe2004} but ported in three dimensions~\cite{Urschler2006, Cheung2009}. However we are not using the feature characterisation and matching (covered in the United States by US Patent 6,711,293) that would be trivial for our spherical objects. We stress that volumetric implementation of \textsc{sift} implies a large amount of data (typically more than \unit{1}{\giga b} for a $(\unit{256}{px})^3$ picture) and thus require careful memory management.
