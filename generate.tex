...
\begin{figure}
	...
	\begin{tikzpicture}[
		...
		\addplot table[x index=0, y index=3]{large_small_mrco_t000.gauss.zhist};
		...
		\addplot table[x index=0, y index=3]{large_small_mrco_t200.gauss.zhist};
		...
		\end{groupplot}
	\end{tikzpicture}
	\caption{\textbf{Instantaneous density profiles.} Top: large particles ($R>\unit{1.5}{\micro\metre}$) without smoothing (black line) and with a Gaussian smoothing of the size of the particles (gray area). Bottom: small particles ($R<\unit{1.5}{\micro\metre}$) with the same color code. The red curve is the smoothed density of ordered particles ($Q_6>0.25$) irrespective of their size. Left: $t=0$. Right: $t=\unit{40}{\hour}$.}
	\label{fig:profiles}
\end{figure}
