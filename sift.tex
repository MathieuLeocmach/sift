...
Real images suffer from optical limitations, \emph{e.g.} the point-spread function of the microscope. In particular, spherical particles observed by confocal microscopy seem elongated along $Z$. Theoretical expressions of point spread functions are rather complicated, but in practice they are often approximated by an anisotropic Gaussian blur~\cite{Zhang2007}. This blur is present in the original image, so that $\sigma_{z,0} = \beta\sigma_{x,0} = \beta\sigma_{y,0}$. Further Gaussian blurring (see Eq.~(\ref{eq:sigma_i})) keeps the ratio $\beta$ constant. We show in Appendix~\ref{sec:gaussian_vs_ball} that the radius of an isolated ball is still proportional to the optimal scale $\sigma$
\begin{equation}
	R = \sigma \sqrt{\frac{(2+\beta) \log 2}{n(1-2^{-2/n})}}. 
	\label{eq:scale_dil_aniso}
\end{equation}

All the equations of the previous section can be adapted according to Eq.~(\ref{eq:aniso}). In particular neighbouring particles are more coupled (effectively closer by a factor $\beta$) if they are aligned along the $Z$ direction.

More problematic is the loss of precision in the localisation along $Z$. We found that the distance between two isolated particle in contact along $Z$ is underestimated.
