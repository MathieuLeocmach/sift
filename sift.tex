...
Under such conditions, the particles are detected smaller than their expected size. We estimate the hard-core diameter function of the detected radius $R$ by locating the first peak of the partial radial distribution function $g_R(r)$ of the particles having $R_i$ close to $R$. As shown in Fig.~\ref{fig:sizing}, the ratio between the hard-core radius and the optical radius can be as large as $1.5$ but is rather constant, thus a single overall scaling factor is enough. It can be determined by a single partial $g_R(r)$ with $R$ near the peak of the size distribution, or by locating the peak of $g(\hat{r})$. We stress that the scaling factor must be applied after, \emph{not} before, finite dilution corrections.
