...
\noindent\LARGE{\textbf{Novel particle tracking method with single-particle size measurement and its application to glassy hard sphere colloids}}
\vspace{0.6cm}

\noindent\large{\textbf{Mathieu Leocmach\textit{$^{a}$} and
Hajime Tanaka$^{\ast}$\textit{$^{b}$}}}\vspace{0.5cm}
...
\noindent \normalsize{We propose a method to localise spherical particles of arbitrary relative sizes from either 2D or 3D (confocal) images either in dilute or crowded environment. Moreover the size of each particle can be estimated reliably. We use this method to analyse the heterogeneous crystallisation in a glassy colloidal hard spheres suspension and found non trivial influence of a tail of small particles on the shape and size of the crystallites.}
\vspace{0.5cm}
 \end{@twocolumnfalse}
  ]
  
\footnotetext{\textit{$^{a}$~Laboratoire de Physique, CNRS UMR 5672, Ecole Normale Supérieure de Lyon, 46 allée d'Italie 69364 Lyon cedex 07, France}}
\footnotetext{\textit{$^{b}$~Institute of Industrial Science, University of Tokyo, 4-6-1 Komaba, Meguro-ku, Tokyo 153-8505, Japan. E-mail: tanaka@iis.u-tokyo.ac.jp}}
